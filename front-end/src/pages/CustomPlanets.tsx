...
export default function CustomPlanets() {
  ...
  const [colorMode, setColorMode] = useState<'terrain' | 'gaseous'>('terrain');
  const [gasType, setGasType] = useState<'ammonia' | 'methane'>('methane');
  ...
  function createPlanetTexture() {
    ...
    for (let i = 0; i < pixelCount; i++) {
      ...
      const address = i * 4;
      buffer[address]   = Math.round(r * 255);
      buffer[address+1] = Math.round(g * 255);
      buffer[address+2] = Math.round(b * 255);
      buffer[address+3] = 255;
    }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
      <aside style={{ width: 260, padding: 16, color: '#eee', overflowY: 'auto' }}>
        ...
        <div>
          <label>Name:</label><br />
          <input type="text" value={planetName} onChange={e => setPlanetName(e.target.value)} />
        </div>
        ...
        <div>
          ...
          <input type="range" min={realisticValues.planetSize.min} max={realisticValues.planetSize.max} step={0.1}
            value={planetSize}
        ...
        <div>
          ...
          <input type="range" min={realisticValues.orbitRadius.min} max={realisticValues.orbitRadius.max} step={0.1}
            value={orbitRadius}
        ...
        <div>
          ...
          <input type="range" min={realisticValues.orbitSpeed.min} max={realisticValues.orbitSpeed.max} step={0.01}
            value={orbitSpeed}
      ...
      <main style={{ flexGrow: 1 }}>
        <Canvas>
          ...
          <OrbitLine radius={orbitRadius * realisticValues.orbitRadius.scale} />
        </Canvas>
      </main>
    </div>
  );
